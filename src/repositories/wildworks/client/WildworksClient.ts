...
import { WildworksClientOptions } from "./WildworksClientOptions";
...
export class WildworksClient extends EventEmitter {
   ...
   */
  public constructor(public readonly options: WildworksClientOptions) {
    super();
  }
  ...
  public async connect(): Promise<void> {
    try {
      ...
      this.socket!.pipe(new DelimiterTransform(0x00))
  ...
  public async write(message: string): Promise<number> {
    return new Promise<number>((resolve, reject) => {
      ...
      const writable =
        this.socket!.write(message) && this.socket!.write("\x00");
      if (writable) {
        cleanup();
  ...
  private async createProxyConnection(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      proxySocket.once("connect", () => {
        proxySocket.write(
          createConnectRequest(this.options.host, this.options.port, true),
        );
      });
      ...
      proxySocket.once("data", (data) => {
        try {
          ...
          if (statusCode === 200) {
            ...
            reject(
              new Error(
                `Proxy connection failed with status code ${statusCode}`,
              ),
            );
          }
        } catch (err) {
